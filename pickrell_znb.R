...
genotype_prediction <- function(x, path){
  ...
  ## count A
  A_est_valset = count_est(scoreA_list, 22.7223876, 3.8590368, 0.5401887)
  gc()

  ## count C
  C_est_valset = count_est(scoreC_list, 27.0894801, 3.1976742, 0.4800347)
  gc()

  ## count G
  G_est_valset = count_est(scoreG_list, 27.3686284, 3.1207975, 0.5019957)
  gc()

  ## count T
  T_est_valset = count_est(scoreT_list, 22.8451061, 4.0265778, 0.5257705)
  gc()
...
save(pred_geno, file = "./rda/pickrell_znb_pred_geno.rda", compress = TRUE)

q(save = "no")

## shell script: echo R CMD BATCH pickrell_znb.R |  qsub -N pic_znb -cwd -pe local 10 -R y -l mf=20G,h_vmem=20G
