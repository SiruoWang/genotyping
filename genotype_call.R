...
load("./rda/training_model_ratio.rda")
...
prediction_prepare <- function(){
  ...
  run_val_id <- which(sra_gtex_blood$SUBJID %in% subjid_val)
...
predict_genotype <- mclapply(c(1:length(sra_path_list[[1]])),
                                    ...
                                    path = sra_path_list,
                                    mc.cores = 10)
save(predict_genotype, file = "./rda/predict_genotype.rda", compress = TRUE)
q(save = "no")


## shell script: echo R CMD BATCH genotype_call_model_prediction.R | qsub -cwd -pe local 20 -R y -l mf=20G,h_vmem=20G
