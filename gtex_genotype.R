...
length(unique(pheno_subset$SUBJID))
length(unique(pheno_subset$Histological_Type))
length(unique(pheno_subset$Body_Site))
...
## full paths for subset gtex samples -- skin
skin_subset <- sra_gtex[which(sra_gtex$Histological_Type == "Skin"),]

path_A_skin <- paste0("find /dcl01/leek/data/gtex_v2/batch_*/coverage_bigwigs/", skin_subset[,"Run"], "*?[0-9].A.bw >> path_A_skin.txt")
write.table(path_A_skin, file = "path_A_skin.sh", row.names = FALSE, col.names = FALSE, quote=FALSE)

path_C_skin <- paste0("find /dcl01/leek/data/gtex_v2/batch_*/coverage_bigwigs/", skin_subset[,"Run"], "*?[0-9].C.bw >> path_C_skin.txt")
write.table(path_C_skin, file = "path_C_skin.sh", row.names = FALSE, col.names = FALSE, quote=FALSE)

path_G_skin <- paste0("find /dcl01/leek/data/gtex_v2/batch_*/coverage_bigwigs/", skin_subset[,"Run"], "*?[0-9].G.bw >> path_G_skin.txt")
write.table(path_G_skin, file = "path_G_skin.sh", row.names = FALSE, col.names = FALSE, quote=FALSE)

path_T_skin <- paste0("find /dcl01/leek/data/gtex_v2/batch_*/coverage_bigwigs/", skin_subset[,"Run"], "*?[0-9].T.bw >> path_T_skin.txt")
write.table(path_T_skin, file = "path_T_skin.sh", row.names = FALSE, col.names = FALSE, quote=FALSE)

path_bw_skin <- paste0("find /dcl01/leek/data/gtex_v2/batch_*/coverage_bigwigs/", skin_subset[,"Run"], "*?[0-9].bw >> path_bw_skin.txt")
